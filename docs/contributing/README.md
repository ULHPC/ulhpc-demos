...
![](https://hpc-docs.uni.lu/contributing/images/github_flow.png)
...
* As preliminaries (to be done only once),  __[Fork](https://docs.github.com/en/free-pro-team@latest/github/getting-started-with-github/fork-a-repo) the `www/ulhpc-demos` repository under `<YOUR-USERNAME>/ulhpc-demos`__
    ...
    - In the top-right corner of the  `www/ulhpc-demos` repository, click "Fork" button.
    ...
    $> make setup
    ```
    - Configure your working forked copy to sync with the original `www/ulhpdemos` repository through a dedicated `upstream` [remote](https://git-scm.com/docs/git-remote)
    ```bash
    # Check current remote: only 'origin' should be listed
    $> git remote -v
    origin  git@github.com:YOUR-USERNAME/ulhpc-demos.git (fetch)
    origin  git@github.com:YOUR-USERNAME/ulhpc-demos.git (push)
    # Add upstream
    $> make setup-upstream
    # OR, manually:
    $> git remote add upstream https://github.com/www/ulhpdemos.git
    # Check the new remote
    $> git remote -v
    origin  git@github.com:YOUR-USERNAME/ulhpc-demos.git (fetch)
    origin  git@github.com:YOUR-USERNAME/ulhpc-demos.git (push)
    upstream https://github.com/www/ulhpdemos.git (fetch)
    upstream https://github.com/www/ulhpdemos.git (push)
    ```

    - At this level, you probably want to follow the [setup](../setup.md) instructions to configure your `ulhpc-demos` python virtualenv and deploy locally the documentation with `make doc`
...
![](https://hpc-docs.uni.lu/contributing/images/gitflow.png)
